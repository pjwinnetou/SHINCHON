...
double Diss::MakeWSource(double tau, int alpha, Grid *grid_pt,
    ...
    if (rk_flag == 0) {
        ...
        music_message.error("rk_flag out of range.");
        exit(0);
    }
    ...
    // final result
    double result = sf*shear_on + bf*bulk_on;

    if (isnan(result)) {
        music_message << "Grid cell at ix = " << grid_pt->position[1]
                      << ", iy = " << grid_pt->position[2]
                      << ", ieta = " << grid_pt->position[3];
        ...
        music_message << "sf=" << sf << " bf=" << bf 
                      ...
                      << " Pimunu[" << rk_flag << "]="
                      << grid_pt->Pimunu[rk_flag][alpha][0]
                      << " prevWmunu=" << grid_pt->prevWmunu[0][alpha][0];
        music_message.flush("error");
    }
    return result;
}/* MakeWSource */
...
// ////////////////////////////////////////////////////////////////////// //
 ...
 if(!isfinite(tau_pi))
 {
     ...
     music_message.error("tau_pi was infinite ...");
 }
...
/* Get_uPis */
 ...
 nmax[3] = DATA->neta-1;
       ...
       else if (direc==3) // eta direction needs special care in mpi:
	 {
   	...
   	else {music_message.error("direc out of range.");exit(0);}
    ...
    double tau, Grid *grid_pt, InitData *DATA, int rk_flag)
{
    ...
    //double shear = (DATA->shear_to_s)*(grid_pt->epsilon + grid_pt->p)/temperature;  

    // cs2 is the velocity of sound squared
    //double cs2 = eos->get_cs2(grid_pt->epsilon, grid_pt->rhob);  
    double cs2 = eos->p_e_func(grid_pt->epsilon, grid_pt->rhob);  
