...
void CAnalyze::CalcFlucQnEbE(){
  ...
  for(ievent=1;ievent<=nevents;ievent++){
    ...
    for (int n=0;n<nmax;n++)
      {
	//Modifications for higher order cumulants -- AH,MH
	...
	if(ispecies==0){
	  ...
	  for(ipt=0;ipt<STAR_PION_NPTBINS;ipt++){
	    ...
	    if(pt>STAR_PION_PTMIN && pt<STAR_PION_PTMAX){
	      fprintf(detailsfile,"%6.4f\t",pt/1000.0);
	      //Modifications for higher order cumulants -- AH,MH
	      //One out of three routines that need to be updated
	      //this loops over the n-th cumulants, not the h.o. cumulants!
	      for (int n=1;n<=nmax;n++)
		{		
		  double r2,i2,normi;
		  //this calculates either v_n{2} or v'_n{2}; why are there mixed Q_n's in the expression: lower and upper case Q_n
		  //(\sum |e^{i \phi} *e^{i \Phi}|^2 -M)/(M(M-1))
		  ...
		  //this is some normalisation constant
		  //(\sum |e^{i \Phi} *e^{i \Phi}|^2 -M)/(M(M-1))
		  normi=(Qnreal[n]*Qnreal[n]+Qnimag[n]*Qnimag[n]-Qnreal[0])/(Qnreal[0]*(Qnreal[0]-1));
		  //this is the actual output, most likely differential v'_n{2}
	...
	if(ispecies==1){
	  ...
	  for(ipt=0;ipt<STAR_KAON_NPTBINS;ipt++){
	    ...
	    if(pt>STAR_KAON_PTMIN && pt<STAR_KAON_PTMAX){
	      fprintf(detailsfile,"%6.4f\t",pt/1000.0);
	      //Modifications for higher order cumulants
	      //Two out of three routines that need to be updated
	      for (int n=1;n<=nmax;n++)
		{		
		  double r2,i2,normi;
		  r2=(qnreal[ispecies][n][ipt]*Qnreal[n]+qnimag[ispecies][n][ipt]*Qnimag[n]-qnreal[ispecies][0][ipt])/(qnreal[ispecies][0][ipt]*(Qnreal[0]-1));
	...
	if(ispecies==2){
	  ...
	  for(ipt=0;ipt<STAR_PROTON_NPTBINS;ipt++){
	    ...
	    if(pt>STAR_PROTON_PTMIN && pt<STAR_PROTON_PTMAX){
	      fprintf(detailsfile,"%6.4f\t",pt/1000.0);
	      //Modifications for higher order cumulants
	      //Three out of three routines that need to be updated
	      for (int n=1;n<=nmax;n++)
		{		
		  double r2,i2,normi;
		  r2=(qnreal[ispecies][n][ipt]*Qnreal[n]+qnimag[ispecies][n][ipt]*Qnimag[n]-qnreal[ispecies][0][ipt])/(qnreal[ispecies][0][ipt]*(Qnreal[0]-1));
