	  ...
	  for(int iy=0; iy<N; iy++)
	    {
	      ...
	      // j.nagle - 04/19/2019 - modification for smooth Gaussian of color charge
	      double gaussigma_x = 2.0;  // in fm
	      double gaussigma_y = 1.0;  // in fm
	      double xx = -L/2.+a*(double)ix;
	      double yy = -L/2.+a*(double)iy;
	      double g2mu20 = param->getg2mu()*param->getg2mu()/param->getg()/param->getg();
	      //double gausweight = exp( - (xx*xx + yy*yy) / (2.0 * gaussigma * gaussigma) );
	      double gausweight = exp( - xx*xx/(2.0*gaussigma_x*gaussigma_x) - yy*yy/(2.0*gaussigma_y*gaussigma_y) );
				//double g2mu2  = g2mu20 * gausweight;
				double g2mu2  = 0.5 * g2mu20 * gausweight;

				//cout << ix << " " << iy << " " << g2mu2 << endl;

				lat->cells[localpos]->setg2mu2A(g2mu2);
				lat->cells[localpos]->setg2mu2B(g2mu2);
  ...
  for (int i = 0; i<A1; i++) 
    {
      foutNpart << nucleusA.at(i).x << " " << nucleusA.at(i).y << " " << nucleusA.at(i).proton << " " << nucleusA.at(i).collided << endl;
			if ( param->getUseConstituentQuarkProton()>0 ){
				for (int iq=0; iq<param->getUseConstituentQuarkProton(); iq++){
					foutNpart << xq[i][iq] << " " << yq[i][iq] << endl; 
				}//ii
			}
    }
  foutNpart << endl;
  for (int i = 0; i<A2; i++) 
    {
      foutNpart << nucleusB.at(i).x << " " << nucleusB.at(i).y << " " << nucleusB.at(i).proton << " " << nucleusB.at(i).collided << endl;
			if ( param->getUseConstituentQuarkProton()>0 ){
				for (int iq=0; iq<param->getUseConstituentQuarkProton(); iq++){
					foutNpart << xq2[i][iq] << " " << yq2[i][iq] << endl; 
				}//ii
			}
    }
  foutNpart.close();
