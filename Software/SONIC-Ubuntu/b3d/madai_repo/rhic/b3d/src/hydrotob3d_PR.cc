...
void CHYDROtoB3D::ReadInputPR(){
	...
	if(input != NULL){
		nprcells=0;
		do{
			prcell[nprcells].Readbulk(input);
			nprcells+=1;
		}while(!feof(input));
		fclose(input);
	}
	  ...
	  {	    
	    printf("%s not found! Trying fallback\n",inputfilename.c_str());
	    //try usual freezeout file
	    string minputfilename="output/"+b3d->run_name+"/"+b3d->qualifier+"/freezeout.dat";
	    printf("freezeout info from %s\n",minputfilename.c_str());
	    input=fopen(minputfilename.c_str(),"r");
	    if(input != NULL){
	      nprcells=0;
	      do{
...
void CPRCell::Readbulk(FILE *fptr){
  double pixx_overh,pixy_overh,piyy_overh,pib_overh,zsize;
	int idirection;
	fscanf(fptr,"%lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf",
	       &x,&y,&tau,&idirection,&ux,&uy,&pixx_overh,&pixy_overh,&piyy_overh,&pib_overh,&T);
	Omega_x=Omega_y=Omega_0=0.0;
	zsize=2.0*etamax*tau;
	//printf("zsize=%g, etamax=%g, tau=%g\n",zsize,etamax,tau);
	//printf("omega_xyspacing+%g, omega_tauspacing=%g\n",omega_xyspacing,omega_tauspacing);
	...
	//printf("Omega=(%g,%g,%g)\n",Omega_0,Omega_x,Omega_y);
	GetPiTildeOverH(pixx_overh,pixy_overh,piyy_overh,pib_overh);
}

void CPRCell::GetPiTildeOverH(double pixx,double pixy,double piyy,double pib){ // This gets Pi/h in fluid frame from pixx/h,pixy/h,piyy/h, ux,uy
	// then boosts to get PiTilde
	double Pi[4][4],u[4];
	double u0=sqrt(1.0+ux*ux+uy*uy);
	u[0]=u0;
	...
	u[2]=uy;
	u[3]=0.0;
	Pi[0][0]=(ux*ux*pixx+2*ux*uy*pixy+uy*uy*piyy)/(u0*u0)+(1-u0*u0)*pib;
	Pi[0][1]=(ux*pixx+uy*pixy)/u0-u0*ux*pib;
	Pi[0][2]=(ux*pixy+uy*piyy)/u0-u0*uy*pib;
	Pi[0][3]=0.0;
	Pi[1][0]=Pi[0][1];
	Pi[1][1]=pixx-(1+ux*ux)*pib;
	Pi[1][2]=pixy-ux*uy*pib;
	Pi[1][3]=0.0;
	Pi[2][0]=Pi[0][2];
	Pi[2][1]=Pi[1][2];
	Pi[2][2]=piyy-(1+uy*uy)*pib;
	Pi[2][3]=0.0;
	Pi[3][0]=0.0;
	Pi[3][1]=0.0;
	Pi[3][2]=0.0;
