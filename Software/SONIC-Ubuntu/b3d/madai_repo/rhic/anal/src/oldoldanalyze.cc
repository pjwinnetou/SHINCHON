...
void CAnalyze::SetQualifier(string qualifiername){
	...
	detailsfilename="analysis/"+run_name+"/"+qualifier+"/details.dat";
	bool WRITEDETAILS=parameter::getB(parmap,string("WRITEDETAILS"),false);
	if(WRITEDETAILS){
		FILE* detailsfile=fopen(detailsfilename.c_str(),"w");
		printf("Will write details to %s\n",detailsfilename.c_str());
		fprintf(detailsfile,"#-------- %s ----------\n",qualifier.c_str());
		fflush(detailsfile);
		fclose(detailsfile);
	}
	//else printf("Won't write details\n");
