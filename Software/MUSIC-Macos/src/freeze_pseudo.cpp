            ...
            for (int ipt = 0; ipt < iptmax; ipt++) {
                for (int iphi = 0; iphi < iphimax; iphi++) {
                    ...
                    if (status == 0 && DATA->echo_level > 9) {
                        music_message << "fscan status = " << status;
...
void Freeze::compute_thermal_spectra(
        int particleSpectrumNumber, InitData* DATA, int size, int rank) {
    double mass_tol = 1e-3;
    double mu_tol = 1e-3;
    int alreadyread = 0;
    if (size > (DATA->pseudo_steps + 1)) {
        music_message << "Cannot run Cooper-Frye with " << DATA->pseudo_steps
    ...
    if (rank == 0) {
        int status = system("rm yptphiSpectra.dat yptphiSpectra?.dat "
                            "yptphiSpectra??.dat particleInformation.dat "
                            "2> /dev/null");
        if (status != 0 && DATA->echo_level > 9) {
            music_message << "system status = " << status;
            music_message.flush("warning");
        }
    }
    ...
    if (particleSpectrumNumber == 0) {
        ...
        for (int i = 1; i < particleMax_copy; i++) {
            ...
            for(int part = 1; part < i; part++) {
                ...
                if (mass_diff < mass_tol && mu_diff < mu_tol
                        && abs(particleList[i].baryon)
                              == abs(particleList[part].baryon)) {
            ...
            if (computespectrum) {
                ...
                // and concatonate results
                // (Don't want any processors to start writing the next
                // particle to file until the concatonation is done)
                MPI_Barrier(MPI_COMM_WORLD);
        
                if (rank == 0) {
                    int status = system("cat yptphiSpectra?.dat "
                                        ">> yptphiSpectra.dat");
                    status = system("cat yptphiSpectra??.dat "
                                    ">> yptphiSpectra.dat 2> /dev/null");
                    if (status != 0 && DATA->echo_level > 9) {
                        music_message << "system status = " << status;
                        music_message.flush("warning");
                    }
                ...
                MPI_Barrier(MPI_COMM_WORLD);
                // occasionally another processor will open file
                // before concatonation is done if this is removed
            }
        }
    } else {
        ...
        // send something just to make sure that rank 0
        // waits for all others to be done:
        int check;
        check = rank;
    
        if (rank > 0) {
            MPI_Send(&check, 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
        }
    
        if (rank == 0) {
            for (int from = 1; from < size; from ++) {
                MPI_Recv(&check, 1, MPI_INT, from, 1,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            int status = system("cat yptphiSpectra?.dat >> yptphiSpectra.dat");
            status = system("cat yptphiSpectra??.dat >> yptphiSpectra.dat "
                            "2> /dev/null");
            if (status != 0 && DATA->echo_level > 9) {
                music_message << "system status = " << status;
                music_message.flush("warning");
...
void Freeze::perform_resonance_decays(int mode, InitData *DATA, int rank) {
    ...
    if (rank == 0) {
        int status = system("rm FyptphiSpectra.dat FparticleInformation.dat "
                            "2> /dev/null");
        if (status != 0 && DATA->echo_level > 9) {
            music_message << "system status = " << status;
            music_message.flush("warning");
...
void Freeze::CooperFrye_pseudo(int particleSpectrumNumber, int mode,
    ...
    if (mode == 3 || mode == 1) {  // compute thermal spectra
        compute_thermal_spectra(particleSpectrumNumber, DATA, size, rank);
    }
    if (mode==4 || mode==1) { //  do resonance decays
        perform_resonance_decays(mode, DATA, rank);
    } 
