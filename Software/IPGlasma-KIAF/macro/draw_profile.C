...
void draw_profile(const float length=30., const int ngrid=512, const int event=0, const int nA=197, const int nB=197, const int nQA=0, const int nQB=0){
	...
	while ( flist >> fname ){
		...
		string buf_sub = buf_fname.substr(buf_fname.find("-t")+2,buf_fname.find("-0.dat")-buf_fname.find("-t")-2);
		...
		while ( fdata >> eta >> x >> y >> u
			...
			h2_time_bin[count]->Fill(x+0.5*width, y+0.5*width);
