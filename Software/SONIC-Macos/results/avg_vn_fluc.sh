...
outname=$(printf '%s/averages/av_%s_vn.dat' $adir $species)
allnum="$adir/data_*"

echo "Averaging over flow coefficients stemming from fluctuations: ..._vn_fluc.dat"
echo "Current arguments start: ./avg_vn_fluc.sh $adir $species"
echo "Output: $outname"
...
do
	...
	file=$(printf '%s/data_%s/results/%s_vn_fluc.dat' $adir $num $species)
