...
void makePlots(int nrun= 1000, int kInitPos = 1, int kDataSel=1, int phiN=2)
{
  const char* fPosStr;
  ...
  for (int istate = 1; istate<=3; istate++){
    TFile* rf = new TFile(Form("outfile_UpsSkim_PhiAng_%s_%d_%s_0000_%04d_%ds.root",fDataStr[kDataSel],phiN,fPosStr,nrun,istate),"read");
    ...
    TFile* wf = new TFile(Form("%s/SavedPlots_%s_PhiAng%d_%s_nRun%d_%ds.root",savedir.c_str(),fDataStr[kDataSel],phiN,fPosStr,nrun,istate),"recreate");
