...
void doFitPhiAng(int nrun = 1000, int InitPos=1, const int fitorder = 2, int kDataSel = 1, int phiN=2)
{
  ...
  for (int istate = 1; istate<=nStates; istate++){
    TFile* rf = new TFile(Form("res/SavedPlots_%s_PhiAng%d_%s_nRun%d_%ds.root",fDataStr[kDataSel],phiN,initPosStr,nrun,istate),"read");

    TFile* wf = new TFile(Form("res/FitResults_v%d_fitorder%d_%s_%s_nRun%d_%ds.root",phiN,fitorder,fDataStr[kDataSel],initPosStr,nrun,istate),"recreate");
