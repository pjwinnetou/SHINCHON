...
void SetGraphAxis(TGraph* g, string xtitle, string ytitle){
...
void SetHistAxis(TH1* h, string xtitle, string ytitle){
...
void SetGraphStyleSys2(TGraph* gr, Int_t c) {
  ...
  gr-> SetLineColor(colorArr[c]);
  gr-> SetFillColorAlpha(colorArrSys[c],0.5);
  if(c==0) gr-> SetFillColorAlpha(colorArrSys[c],0.2);
  gr-> SetLineWidth(1);
}
...
void SetCanvasSquare(TCanvas *c) {
    c->SetLeftMargin(0.12);
    c->SetRightMargin(0.14);
    c->SetBottomMargin(0.1);
    c->SetTopMargin(0.064);
}

void SetCanvasSquare2(TCanvas *c) {
  c->SetLeftMargin(0.18);
  c->SetBottomMargin(0.16);
  c->SetTopMargin(0.064);
}

void SetCanvasSquare3(TCanvas *c) {
...
TCanvas *makeHistRatioCanvas(TH1* h0, TH1 *h1, TH1 *h2, double rup, double rdo, bool logopt){
  ...
  ymax = ymax*1.1;
  ymin = ymin*0.9;
  h1->GetYaxis()->SetLimits(ymin,ymax);
...
//  pad2->SetRightMargin(0.05);
  ...
  pad2->cd();
  h0->Draw("pe");
  dashedLine(h0->GetXaxis()->GetXmin(),1,h0->GetXaxis()->GetXmax(),1,1);
  pad2->Update();
  c1->Modified();
  c1->Update();
