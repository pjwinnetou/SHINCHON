...
void drawRAA_pt_compare_data(int nrun = 10, int kInitPos=1, bool isLine = true){
  ...
  //CMS data
  TFile* fCMS = new TFile("../DataFiles/CMS_RAA_pt.root","read");
  TH1D* hCMS_RAA = (TH1D*) fCMS->Get("Table 13/Hist1D_y1");
  TH1D* hCMS_RAA_stat = (TH1D*) fCMS->Get("Table 13/Hist1D_y1_e1");
  TH1D* hCMS_RAA_sys = (TH1D*) fCMS->Get("Table 13/Hist1D_y1_e2");

  TGraphErrors* gRAA_cms = new TGraphErrors();
  TGraphAsymmErrors* gRAA_cms_sys = new TGraphAsymmErrors();
  for(int ipt=0; ipt<hCMS_RAA->GetNbinsX(); ipt++){
    gRAA_cms->SetPoint(ipt, hCMS_RAA->GetBinCenter(ipt+1), hCMS_RAA->GetBinContent(ipt+1));
    gRAA_cms->SetPointError(ipt, 0, hCMS_RAA_stat->GetBinContent(ipt+1));
    gRAA_cms_sys->SetPoint(ipt, hCMS_RAA->GetBinCenter(ipt+1), hCMS_RAA->GetBinContent(ipt+1));
    gRAA_cms_sys->SetPointError(ipt, hCMS_RAA->GetBinWidth(ipt+1)/2,hCMS_RAA->GetBinWidth(ipt+1)/2, hCMS_RAA_sys->GetBinContent(ipt+1), hCMS_RAA_sys->GetBinContent(ipt+1));
  }
  SetGraphStyleOpen(gRAA_cms,4,0,0);
  SetGraphStyleSys2(gRAA_cms_sys,0);
  ...
  c1->SaveAs(Form("%s/compare_RAA_vs_pt_data_isLine%d.pdf",savedir.c_str(),isLine));
